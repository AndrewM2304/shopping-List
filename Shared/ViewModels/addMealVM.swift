...
//        mealObject = Meal(context: managedObjectContext)
    ...
    func addDate(managedObjectContext: NSManagedObjectContext, dateEntry: Date, mealtypeEntry: mealTypeName) {
        withAnimation {
            ...
            if(newDate.mealTypeNameStatus == .breakfast){
                newDate.sortedOrder = "a"
            } else if(newDate.mealTypeNameStatus == .lunch){
                newDate.sortedOrder = "b"
            } else {
                newDate.sortedOrder = "c"
            }
            
            if(!dateList.contains(newDate)){
            dateList.append(newDate)
                print("\(newDate.mealTypeNameStatus), \(newDate.date!) added")
            } else{
    ...
    func checkAndSelect(meal: Meal?){
        
        if(meal == nil){
            return
        } else {
            for list in meal!.ingredientArray{
    ...
    func saveMeal(managedObjectContext: NSManagedObjectContext, myMealName: String, myMealLink: String, myMealNotes: String) {
            ...
            newMeal.mealLink = myMealLink
            newMeal.mealNotes = myMealNotes
